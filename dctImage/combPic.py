...
def combin(pic_path,target_path) -> str :
    # 分割后的图片的文件夹，以及拼接后要保存的文件夹
    pic_path = pic_path
    pic_target = target_path
    ...
    if len(picture_names) == 0:
        ...
        # 获取分割后图片的尺寸
        img_1_1 = cv2.imread(pic_path + '1_1.bmp')
        ...
        for i in range(1, num_width + 1):
            for j in range(1, num_length + 1):
                img_part = cv2.imread(pic_path + '{}_{}.bmp'.format(i, j))
                splicing_pic[width * (i - 1): width * i, length * (j - 1): length * j, :] = img_part
        # 保存图片，大功告成
        cv2.imwrite(pic_target + 'result.bmp', splicing_pic)
    return pic_target + 'result.bmp'
